...
impl Statistics {
    ...
    pub fn print_last_frame(&self, w: &mut dyn io::Write) -> io::Result<()> {
        ...
        let last_10: Duration = self
            ...
            .rev()
            .take(10)
            .map(|it| it.frame_time)
            .sum();
        writeln!(
            w,
            "    current: {:4}ms = {:6.2}f/s",
            frame.frame_time.as_millis(),
            1.0 / frame.frame_time.as_secs_f64(),
        )?;
        writeln!(
            w,
            "    last 10: {:4}ms = {:6.2}f/s",
            last_10.as_millis(),
        ...
        writeln!(
            w,
            "Player: at ({:6.1}, {:6.1}, {:6.1}) facing ({:6.3}, {:6.3}, {:6.3})",
            p.x, p.y, p.z, o.x, o.y, o.z
        )?;

        let (prev_cic, prev_cmic) = self
            .frame_infos
            .iter()
            .nth_back(1)
            .map(|it| (it.chunk_info_count, it.chunk_mesh_info_count))
            .unwrap_or((0, 0));
        ...
        writeln!(w, "Chunks:")?;
        {
            writeln!(
                w,
                "    total: {:4} generated, {:6.2}ms total, {:6.2}ms average",
            ...
            let chunk_infos_duration: f64 =
                chunk_infos.iter().map(|it| it.time.as_secs_f64()).sum();
            if chunk_infos.is_empty() {
                writeln!(w, "    frame: 0 generated")?;
            } else {
                writeln!(
                    w,
                    "    frame: {:4} generated, {:6.2}ms total, {:6.2}ms average",
                    chunk_infos.len(),
        ...
        writeln!(w, "Chunk meshes:")?;
        {
            writeln!(
                ...
                1000.0 * self.total_chunk_mesh_time.as_secs_f64()
                    / self.chunk_mesh_infos.len() as f64,
            )?;
            let chunk_mesh_infos = &self.chunk_mesh_infos[prev_cmic..frame.chunk_mesh_info_count];
            let chunk_mesh_infos_duration: f64 = chunk_mesh_infos
                .iter()
                .map(|it| it.time.as_secs_f64())
                .sum();
        ...
        writeln!(
            w,
            "Statistics: {:.2}ms printing time, {:.3}kB total size",
